...
const CLASES_AUTH_KEY = 'clases_online_auth'

export function getClasesAuth() {
  try {
    const raw = sessionStorage.getItem(CLASES_AUTH_KEY)
...
export function saveClasesAuth(data) {
  sessionStorage.setItem(CLASES_AUTH_KEY, JSON.stringify(data))
}

export function clearClasesAuth() {
  sessionStorage.removeItem(CLASES_AUTH_KEY)
}

export default function ClasesLoginPage({ onLogin }) {
  ...
  const handleSubmit = async (e) => {
    ...
    const auth = { cedula: cedula.trim(), phone4: phone4.trim(), students: data }
    saveClasesAuth(auth)
    onLogin(auth)
    setLoading(false)
  }
  ...
  return (
    ...
    >
      ...
      <div className="text-center mb-8">
        ...
        />
        <h1 className="text-2xl font-bold text-white tracking-tight">Clases Online</h1>
        <p className="text-white/60 text-sm mt-1">Accede a tu clase grabada del día</p>
      </div>
