import { useState } from 'react'
import { X, AlertTriangle, Trash2, Ban, Lock, UserMinus } from 'lucide-react'
import Modal from './ui/Modal'
import { verifyPin } from '../lib/pinUtils'
import { sanitizeError } from '../lib/errorUtils'

const WITHDRAW_REASONS = [
  { value: 'economico', label: 'Económico' },
  { value: 'horario', label: 'Horario no compatible' },
  { value: 'cambio_ciudad', label: 'Cambio de ciudad / país' },
  { value: 'salud', label: 'Salud' },
  { value: 'personal', label: 'Motivo personal' },
  { value: 'otro', label: 'Otro' },
]

export default function DeleteConfirmModal({
  isOpen,
  onClose,
  onConfirm,
  itemName,
  itemType = 'registro',
  requiredPin
}) {
  const [pin, setPin] = useState('')
  const [error, setError] = useState('')
  const [loading, setLoading] = useState(false)
  const [reason, setReason] = useState('')
  const [step, setStep] = useState('confirm') // 'confirm' | 'reason' for alumno

  const isVoid = itemType === 'venta'
  const isAlumno = itemType === 'alumno'
  const actionLabel = isVoid ? 'Anular' : isAlumno ? 'Dar de baja' : 'Eliminar'
  const ActionIcon = isVoid ? Ban : isAlumno ? UserMinus : Trash2

  const handleSubmit = async (e) => {
    e.preventDefault()
    setError('')

    if (requiredPin) {
      const valid = await verifyPin(pin, requiredPin)
      if (!valid) {
        setError('PIN incorrecto')
        setPin('')
        return
      }
    }

    setLoading(true)
    try {
      await onConfirm(isAlumno ? reason : undefined)
      setPin('')
      setReason('')
      setStep('confirm')
      onClose()
    } catch (err) {
      setError(sanitizeError(err))
    } finally {
      setLoading(false)
    }
  }

  const handleClose = () => {
    setPin('')
    setError('')
    setReason('')
    setStep('confirm')
    onClose()
  }

  return (
    <Modal isOpen={isOpen} onClose={handleClose} ariaLabel={`Confirmar ${actionLabel.toLowerCase()}`}>
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm">
        {/* Header */}
        <div className={`px-5 py-4 border-b rounded-t-2xl flex items-center justify-between ${
          isVoid ? 'bg-amber-50' : isAlumno ? 'bg-orange-50' : 'bg-red-50'
        }`}>
          <div className="flex items-center gap-2">
            <AlertTriangle className={isVoid ? 'text-amber-500' : isAlumno ? 'text-orange-500' : 'text-red-500'} size={18} />
            <span className={`font-medium text-sm ${isVoid ? 'text-amber-700' : isAlumno ? 'text-orange-700' : 'text-red-700'}`}>{actionLabel}</span>
          </div>
          <button onClick={handleClose} aria-label="Cerrar" className={`p-2 rounded-xl active:scale-95 transition-all ${
            isVoid ? 'hover:bg-amber-100' : isAlumno ? 'hover:bg-orange-100' : 'hover:bg-red-100'
          }`}>
            <X size={16} className={isVoid ? 'text-amber-500' : isAlumno ? 'text-orange-500' : 'text-red-500'} />
          </button>
        </div>

        {/* Content */}
        <form onSubmit={handleSubmit} className="p-6">
          <div className="text-center mb-4">
            <p className="text-xs text-gray-500">
              {isVoid
                ? '¿Anular esta venta? El stock se restaurará automáticamente.'
                : isAlumno
                  ? 'La alumna pasará a inactiva. Podrás reactivarla cuando regrese.'
                  : '¿Eliminar?'}
            </p>
            <p className="font-semibold text-gray-800 mt-1">{itemName}</p>
          </div>

          {/* Motivo de retiro (solo alumno) */}
          {isAlumno && (
            <div className="mb-4">
              <label className="block text-xs text-gray-500 mb-1.5">Motivo de retiro <span className="text-gray-400">(opcional)</span></label>
              <select
                value={reason}
                onChange={(e) => setReason(e.target.value)}
                className="w-full px-3 py-2 border border-gray-200 rounded-xl text-sm text-gray-700 bg-white focus:ring-2 focus:ring-orange-200 focus:border-orange-400 transition-all"
              >
                <option value="">Sin especificar</option>
                {WITHDRAW_REASONS.map(r => (
                  <option key={r.value} value={r.value}>{r.label}</option>
                ))}
              </select>
            </div>
          )}

          {/* PIN */}
          {requiredPin && (
            <div className="mb-3">
              <label className="block text-xs text-gray-500 mb-1 flex items-center gap-1">
                <Lock size={10} /> PIN de seguridad
              </label>
              <input
                type="password"
                inputMode="numeric"
                maxLength={6}
                value={pin}
                onChange={(e) => {
                  setPin(e.target.value.replace(/\D/g, ''))
                  setError('')
                }}
                className={`w-full px-3 py-2 border-2 rounded-xl text-center text-lg tracking-widest transition-all ${
                  error ? 'border-red-300' : 'border-gray-200'
                }`}
                placeholder="••••"
                autoFocus={!isAlumno}
              />
              {error && <p className="text-red-500 text-xs mt-1 text-center">{error}</p>}
            </div>
          )}

          {/* Botones */}
          <div className="flex gap-3">
            <button
              type="button"
              onClick={handleClose}
              className="flex-1 px-3 py-3 border border-gray-300 text-gray-700 rounded-xl hover:bg-gray-50 active:scale-95 transition-all text-sm font-semibold"
            >
              Cancelar
            </button>
            <button
              type="submit"
              disabled={loading || (requiredPin && pin.length < 4)}
              className={`flex-1 px-3 py-3 rounded-xl text-sm font-semibold flex items-center justify-center gap-1 shadow-sm transition-all active:scale-95 ${
                loading || (requiredPin && pin.length < 4)
                  ? 'bg-gray-300 text-gray-500 cursor-not-allowed'
                  : isVoid
                    ? 'bg-amber-600 hover:bg-amber-700 text-white'
                    : isAlumno
                      ? 'bg-orange-600 hover:bg-orange-700 text-white'
                      : 'bg-red-600 hover:bg-red-700 text-white'
              }`}
            >
              <ActionIcon size={14} />
              {loading ? (isVoid ? 'Anulando...' : isAlumno ? 'Dando de baja...' : 'Eliminando...') : actionLabel}
            </button>
          </div>
        </form>
      </div>
    </Modal>
  )
}
